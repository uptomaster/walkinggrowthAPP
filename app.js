(function () {
  ...
  function doLogin() {
    ...
    // API_BASE 확인
    if (!API_BASE) {
      console.error('API_BASE가 설정되지 않았습니다!');
    ...
    // 모바일 앱에서는 추가 헤더 설정
    if (isMobileApp) {
      fetchOptions.headers['User-Agent'] = 'WalkStory-Mobile/1.0';
    }
      ...
      if (!Kakao.isInitialized()) {
        if (typeof window !== 'undefined' && window.APP_CONFIG && window.APP_CONFIG.kakao) {
          try {
            ...
            showToast('카카오 SDK 초기화에 실패했어요.');
            return;
          }
        } else {
          showToast('카카오 설정이 없어요.');
          return;
        }
      }
  ...
  function sendSocialLogin(provider, socialId, nickname, email) {
    console.log('sendSocialLogin called:', { provider, socialId: socialId?.substring(0, 10) + '...', nickname: nickname?.substring(0, 20), email: email ? 'provided' : 'null', API_BASE: API_BASE });
    
    // API_BASE 확인
    if (!API_BASE) {
      console.error('API_BASE가 설정되지 않았습니다.');
      showToast('서버 연결 설정 오류가 발생했어요.');
      hideAuthLoading();
      return Promise.resolve({ ok: false, body: { error: 'API_BASE가 설정되지 않았습니다.' } });
    }
    ...
    return fetch(url, {
      ...
      console.error('Error details:', { message: err.message, stack: err.stack, url: url, API_BASE: API_BASE });
