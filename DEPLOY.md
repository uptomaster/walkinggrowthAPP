...
4. **Environment** 탭에서 변수 추가:
   ...
   - (선택) `DATABASE_PATH`: `./walk_growth.db` (기본값 사용 가능)
...
필요 시 Render 대시보드에서 Build/Start Command, 환경 변수를 한 번 더 확인하세요.
...
2. **Environment Variables**에서 추가:
   - `JWT_SECRET`: 임의의 긴 비밀 문자열
   - `DATABASE_PATH`: **`/tmp/walk_growth.db`** (Vercel은 쓰기 가능한 경로가 `/tmp`만 가능)
3. **Deploy** 실행.
...
- SQLite DB는 `/tmp`에 저장되므로 **서버리스 콜드 스타트·재배포 시 데이터가 초기화**될 수 있습니다.  
  장기 보존이 필요하면 Render 사용 또는 외부 DB(예: PostgreSQL) 연동을 권장합니다.
